...
class RelativeMultiheadAttention(nn.MultiheadAttention):
    ...
    def _get_relative_embeddings(self, relative_embeddings, length):
        ...
        slice_end_position = min(slice_start_position + length, length+1)
        if pad_length > 0:
...
### Never Use Decoder, Although VITS write it. 
  ...
  def _get_relative_embeddings(self, relative_embeddings, length):
    ...
    if pad_length > 0:
      padded_relative_embeddings = F.pad(
          relative_embeddings,
          commons.convert_pad_shape([[0, 0], [pad_length, pad_length], [0, 0]])) #每一个维度左边填充和右边填充
    else:
      padded_relative_embeddings = relative_embeddings
    used_relative_embeddings = padded_relative_embeddings[:,slice_start_position:slice_end_position]
